 ...
 */
        ...
        /**
         *  每个session的单跳页面切片生成，以及页面流的匹配
         *
         *  假如使用者指定的页面流是3,2,5,8,6
         *  计算得到的pageSplitPvMap数据key为  3->2，2->5，5->8，8->6 , value为页面切片访问次数
         *  <3->2, 1>  <2->5, 1>  <5->8, 1> ...
         */
        ...
        //计算起始页面的访问数( 计算页面切片转化率,需要先得到起始页面的访问数)
        ...
        return sessionid2actionsRDD.flatMapToPair(
                ...
                new PairFlatMapFunction<Tuple2<String,Iterable<Row>>, String, Integer>() {
                    ...
                    public Iterable<Tuple2<String, Integer>> call(Tuple2<String, Iterable<Row>> tuple)throws Exception {
                      ...
                      /**
                        这里拿到的session的访问行为，默认情况下是乱序的,我们希望拿到的数据，是按照时间顺序排序的
